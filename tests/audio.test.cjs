...
test('audio bus mutes every loop, cancels speech on pause, resumes PA, and resets a new duty',async()=>{
 ...
 const gains=[],sources=[],utterances=[],generated=[],requests=[],timers=new Map();let nextTimer=0,cancelled=0;
 ...
 class Node {gain=new Param();frequency=new Param();playbackRate=new Param();connect(node){this.connected=node;}disconnect(){this.disconnected=true;}start(){this.started=true;}stop(){this.stopped=true;this.onended?.();}}
 class Context {
  currentTime=0;destination={};
  createBuffer(channels,length){const data=new Float32Array(length);generated.push(data);return {getChannelData:()=>data};}
  resume(){return Promise.resolve();}
  createGain(){const n=new Node();n.kind='gain';gains.push(n);return n;}
 ...
 try {
  ...
  global.fetch=async url=>{requests.push(url);return {ok:true,arrayBuffer:async()=>fs.readFileSync(`public${url}`).buffer};};
  ...
  assert.equal(requests.length,0);assert.equal(sources.length,0,'import does not start Web Audio');
  const initial=initialState();audio.start(initial);
  await new Promise(resolve=>setImmediate(resolve));
  assert.equal(sources.filter(s=>s.loop).length,5);
  assert.equal(requests.length,9,'no new downloaded samples');
  assert.ok(generated.reduce((sum,data)=>sum+data.byteLength,0)<=128*1024);
  assert.ok(generated.every(data=>data.some(v=>Math.abs(v)>.01)&&data.every(v=>Math.abs(v)<1)),'seeded textures are audible and bounded');
  ...
  const oldCompletion=utterances[1].onend;
  ...
  oldCompletion();audio.setActive(false);audio.setActive(true);
  assert.equal(timers.size,1,'an old speech completion cannot clear resumed PA');
  audio.setMute(true);assert.equal(master.gain.value,0);assert.equal(timers.size,0,'mute clears delayed speech');
  assert.ok(sources.filter(s=>!s.loop).every(s=>s.stopped&&s.disconnected));
  const shots=sources.length;
  audio.update({...approach,doors:true,speed:0,dwell:2,stops:1},'coast');
  assert.equal(sources.length,shots,'muted door and boarding events allocate no one-shots');
  audio.setActive(false);audio.setActive(true);assert.equal(master.gain.value,0,'resuming cannot bypass mute');
  audio.setMute(false);assert.equal(master.gain.value,.55);assert.equal(timers.size,0,'unmute never replays a muted announcement');
  audio.start(initial);await new Promise(resolve=>setImmediate(resolve));
  assert.equal(sources.filter(s=>s.loop).length,5,'restart does not duplicate loops');
  for(const source of sources.filter(s=>s.loop)) {
   let output=source.connected;while(output.kind!=='gain')output=output.connected;
   assert.equal(output.gain.value,0,'restart clears prior motor/rail/pass/platform gain');
  }
  assert.equal(requests.length,9,'restart reuses the pack');
...
test('audio failure leaves controls usable and uses the synthesized tone fallback',async()=>{
 ...
 let oscillators=0;const tones=[];
 class Param{cancelScheduledValues(){}setTargetAtTime(){}setValueAtTime(){}exponentialRampToValueAtTime(){}}
 class Node{gain=new Param();frequency=new Param();playbackRate=new Param();connect(){}disconnect(){}start(){}stop(when){if(when===undefined){this.cancelled=true;this.onended?.();}}}
 try{
  global.window={};global.fetch=async()=>{throw new Error('offline');};
  global.AudioContext=class{currentTime=0;destination={};createBuffer(channels,length){return {getChannelData:()=>new Float32Array(length)};}createBufferSource(){return new Node();}createGain(){return new Node();}createOscillator(){oscillators++;const n=new Node();tones.push(n);return n;}createBiquadFilter(){return new Node();}resume(){return Promise.resolve();}};
  const {audio}=load('audio'),{initialState}=load('simulation');
  ...
  audio.setMute(true);assert.ok(tones.slice(1).every(n=>n.cancelled),'mute cancels scheduled fallback tones too');audio.setActive(false);
 }finally{for(const[key,value]of Object.entries(saved)){if(value===undefined)delete global[key];else global[key]=value;}}
});
