...
The running tracks now stream with the corridor: rust-dark webs, bright steel heads, 0.8 m concrete sleepers with rail seats, slab plinths, drainage, covered cable trays and capped, striped parapets. Raised handrails stop before every platform. At-grade lines and depot sidings use dusty ballast shoulders; tunnels retain separate bore floors and gain darker lining joints and cable brackets. Rail gauge, running height and cab coordinates are unchanged.

Landmarks gain podiums, setbacks, roof parapets, AC plant, water tanks, balcony depth, industrial vents and weathered bases. Three overlapping foliage masses and forked trunks reuse the original geometry pool. Shared plaster, concrete, steel, teal glass and foliage finishes respond to the existing lights. One deterministic **128×128 grain texture (64 KiB before mipmaps)** adds mineral wear without downloads; its projection stays fixed to the instances and it is disposed with the shared assets.

For the frame budget, horizon buildings are spaced **80 m** apart with simplified facade bands; detailed facades cap at nine rows and six column intervals. Individual ballast stones, distant window grids, shadow maps and reflection passes are omitted. The palette stays dusty and muted under the existing dusk lighting. These are stylized Bengaluru cues, not photogrammetry. The pass preserves the routes, console CSS, Kanaka, sold Swiggy image, LED rules, cab, physics, audio and liveliness; baseline hashes are checked in `tests/env-fidelity.test.cjs`.

## Performance

[`src/CorridorChunk.tsx`](src/CorridorChunk.tsx) streams a 160 m grid, split at exact hop boundaries: one cell behind and five ahead (at least 640 m ahead throughout a cell). At most nine slices mount across hop/portal edges. Stations mount within 850 m. Solid, concrete, steel, glass, foliage, rounded forms and fluorescent fixtures use shared geometries/materials and instanced batches; each slice's signs share one painted texture atlas and draw call. Instance buffers and sign atlases are disposed on stream-out; shared resources live until the world unmounts. No full-route geometry cache accumulates.

A deterministic check of **every 160 m streaming cell** peaks at **14,209 instances / 54 scenery batches**, including the main track, with at most **2,408 instances in one slice**. This remains inside the original 15,000-instance / 54-batch test limits. Cab, stations, traffic, LED boards and passing train are excluded. These are geometry budgets, not measured GPU timings. The world moves relative to a local cab camera; DPR remains capped at **1.5**, with no shadow maps or postprocessing passes.
...
`npm test` passes **55 individual tests across seven files** (the sandbox runner summarizes files; running each file directly reports the individual cases). The route and audio tests cover the exact 37-station contract, terrain, complete continuous duty, speed cap, inertia, stop tolerance, closing/boarding/traction interlocks, ATP capture across multiple speeds and time steps, all 36 pre-arrival announcements/departures, WAV integrity, mute/pause/resume/restart, and unavailable audio assets. Audio lifecycle tests use Web Audio/Speech mocks; they verify control flow, not audible quality.
...
The eight environment tests cover shared material routing/disposal, deterministic grain and shader injection against the installed Three.js source, rail continuity across arbitrary splits, sleepers and foundations in every layout, platform handrail exclusion, detail limits, every streaming-cell budget and baseline file hashes. The environment pass also inspected software geometry previews of office, mall, yard and tunnel approaches. **Browser verification was attempted and blocked by Chromium's `setsockopt: Operation not permitted` error.** Final WebGL lighting, shader compilation, motion aliasing and desktop/mobile fps still need operator verification; software previews do not establish those results.
...
The first mid-hop LED (hop 0, Whitefield → Hopefarm Channasandra) shows the sold Swiggy “Taste Meets Speed” still creative supplied by Dhivagar; all other mid-hop LEDs remain **ADVERTISE HERE** demo slots, with a procedural **SAMPLE LOOP** on alternating hops. Screens stream with the corridor; at most two animate at 10 fps. To swap content without remeshing, pass `source={{ kind: 'image', url: '/textures/demo.png' }}` to `Billboard`, or `source={{ kind: 'canvas', canvas, paint: seconds => drawFrame(seconds) }}` for animation (including decoded GIF frames). Keep sources stable and load any future GIF decoder only for mounted live slots; a GIF URL alone is a still image slot. Canvas producers own decoder cleanup; boards dispose their GPU textures/materials.
